...
module Linzer
  # ML-DSA-44 (FIPS 204) post-quantum signature algorithm support.
  #
  # ML-DSA is the NIST post-quantum signature standard. Like Ed25519, it is
  # a "pure"/digest-less signature scheme: the RFC 9421 signature base is
  # signed directly, with no prehashing.
  #
  ...
  module MLDSA
    ...
    private_constant :OID

    # ML-DSA-44 signing key implementation.
    #
    # @example Generating a new key pair
    #   key = Linzer.generate_ml_dsa_44_key("my-key-id")
    #
    # @example Loading from PEM
    #   key = Linzer.new_ml_dsa_44_key(File.read("ml_dsa_44.pem"), "key-1")
    #
    # @see Linzer::Key::Helper#generate_ml_dsa_44_key
    # @see Linzer::Key::Helper#new_ml_dsa_44_key
    ...
    class << self
      ...
      def algorithm_identifier
        OpenSSL::ASN1::Sequence.new([OpenSSL::ASN1::ObjectId(OID)])
      end
    end
  end
end
